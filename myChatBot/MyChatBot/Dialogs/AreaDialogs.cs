...
namespace MyChatBot.Dialogs {
	public class AddDialog : ComponentDialog {
		...
		public AddDialog(UserState userState): base(nameof(AddDialog)) {
		...
		private static async Task<DialogTurnResult> GetNameStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken) {
			...
			Context context = JsonConvert.DeserializeObject<Context>(stepContext.Context.Activity.Value.ToString());
			//await stepContext.Context.SendActivityAsync($"What is the name of the new area for {context.lookup.Name}?");
			//return await stepContext.PromptAsync(nameof(ChoicePrompt),
			//	 new PromptOptions {
			//		 Prompt = MessageFactory.Text($"What is the name of the new area for {context.lookup.Name}?"),
			//		 Choices = ChoiceFactory.ToChoices(new List<string> { "Car", "Bus", "Bicycle" }),
			//	 }, cancellationToken);

			stepContext.Values["context"] = stepContext.Context.Activity.Value.ToString();
